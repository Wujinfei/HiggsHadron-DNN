...
def load_data(inputPath,variables,criteria,inputTree):
    ...
    for key in keys :
        ...
        if 'bb' in key:
            ...
            sampleNames=['down']
            fileNames = ['output_z_sl0mu_down']
            target=5
        elif 'up' in key:
            sampleNames=['up']
            fileNames = ['output_z_sl0mu_up']
            target=6
        ...
        ndown = len(data.iloc[(data.target.values == 5) & (data.key.values==key) ])
        nup = len(data.iloc[(data.target.values == 6) & (data.key.values==key) ])
        nOther00 = len(data.iloc[(data.target.values == 7) & (data.key.values==key) ])
        ...
        if key == 'bb':
            ...
            print('Process down frequency: ', len(processfreq.get_group('down')))
        elif key == 'up':
            print('Process up frequency: ', len(processfreq.get_group('up')))
    ...
    ndown = len(data.iloc[data.target.values == 5])
    nup = len(data.iloc[data.target.values == 6])
    nOther = len(data.iloc[data.target.values == 7])
    print("Total length of nbb = %i, ncc = %i, ngg = %i, nww = %i, nzz = %i, ndown = %i, nup = %i" % (nbb, ncc, ngg, nww, nzz, ndown, nup))
    return data
...
def MakePlots(y_train, y_test, y_test_pred, y_train_pred, Wt_train, Wt_test, output):
    ...
    figMVA, axesMVA = plt.subplots(2, 4, figsize=(30, 20))
    ...
    for i in range(7):
        ...
        if i==0:
            ...
            ax=axes[1,2]
            axMVA=axesMVA[1,2]
        nodename=["bb","cc","gg","ww","zz","down","up"]
        ...
        axMVA.hist([y_test_pred[:, i][(y_test[:, 0]==1)],
                    ...
                    y_test_pred[:, i][(y_test[:, 5]==1)],
                    y_test_pred[:, i][(y_test[:, 6]==1)]],bins=np.linspace(0, 1, 21),label=["bb","cc","gg","ww","zz","down","up"],
                   weights=[Wt_test[(y_test[:, 0]==1)]/np.sum(Wt_test[(y_test[:, 0]==1)]),
                            ...
                            Wt_test[(y_test[:, 6]==1)]/np.sum(Wt_test[(y_test[:, 6]==1)])],
        ...
        axMVA.hist([y_train_pred[:, i][(y_train[:, 0]==1)],
                    ...
                    y_train_pred[:, i][(y_train[:, 5]==1)],
                    y_train_pred[:, i][(y_train[:, 6]==1)]],bins=np.linspace(0, 1, 21),label=["bb_train","cc_train","gg_train","ww_train","zz_train","down_train","up_train"],
                   weights=[Wt_train[(y_train[:, 0]==1)]/np.sum(Wt_train[(y_train[:, 0]==1)]),
                            ...
                            Wt_train[(y_train[:, 6]==1)]/np.sum(Wt_train[(y_train[:, 6]==1)])],
